...
def update(frame):
    ...
    if frame <= 240:
        ...
        # add code here if animation length >= 721 frames.
        return
...
def gen_rand_params(n):
    mid_sig = 10
    mid_rho = 28
    mid_beta = round(8/3, 2)
    rand_sig = mid_sig + (mid_sig * 0.6) * np.random.randn(1, n)
    rand_rho = mid_rho + (mid_rho * 0.6) * np.random.randn(1, n)
    rand_beta = mid_beta + (mid_beta * 0.6) * np.random.randn(1, n)
    return rand_sig, rand_rho, rand_beta

rand_sig, rand_rho, rand_beta = gen_rand_params(10)

Xk = [-8, 8, 27]
Xk2 = [-7.9, 8.1, 27.1]

Xk_final_loc = np.empty((3, len(rand_sig)))
Xk2_final_loc = np.empty((3, len(rand_sig)))

for i in range(len(rand_sig)):
    X1, X2 = iter(rand_sig[i], rand_rho[i], rand_beta[i], Xk, Xk2)

    # new final x-position
    Xk_final_loc[i, 0] = X1[0, -1]
    Xk2_final_loc[i, 0] = X2[0, -1]

    # new final y-position
    Xk_final_loc[i, 1] = X1[1, -1]
    Xk2_final_loc[i, 1] = X2[1, -1]

    # new final z-position
    Xk_final_loc[i, 2] = X1[2, -1]
    Xk2_final_loc[i, 2] = X2[2, -1]
